...
<body>
	<div style="color: blue; text-align: center; font-size: 30px;">Orders Received <i class="fas fa-archive"></i></div>
	<%
...
<table id="customers">
	...
	try
	{
		...
		ResultSet rs=st.executeQuery("select * from cart where orderDate is not NULL and status='processing'");
		while(rs.next())
		{     		
	%>       
	<tr>
		...
		<td><a href="cancelOrdersAction.jsp?id=<%=rs.getString(1)%>&email=<%=rs.getString(2)%>">Cancel <i class='fas fa-window-close'></i></a></td>
		<td><a href="deliveredOrdersAction.jsp?id=<%=rs.getString(1)%>&email=<%=rs.getString(2)%>">Delivered <i class='fas fa-dolly'></i></i></a></td>
	</tr>
 <%}
 }
   catch(Exception e)
